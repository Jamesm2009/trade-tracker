...
// Map fund categories from Fund Universe → asset classes
...
export default function PortfolioTab({ data }) {
  ...
  const [allocView, setAllocView] = useState('fund'); // 'fund' or 'class'
  ...
  // Build ticker→category lookup from fund universe
  ...
  // Pick which data to show in the pie
  const pieData = allocView === 'fund' ? allocData : classData;
  const pieColors = allocView === 'fund'
    ? PIE_COLORS
    : classData.map(d => ASSET_CLASS_COLORS[d.name] || '#999');
                  ...
                  <PieChart>
                    ...
                    >
                      {pieData.map((d, i) => (
                        <Cell
                          key={i}
                          fill={allocView === 'class'
                            ? (ASSET_CLASS_COLORS[d.name] || '#999')
                ...
                {pieData.map((d, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{
                      ...
                      background: allocView === 'class'
                        ? (ASSET_CLASS_COLORS[d.name] || '#999')
                        : PIE_COLORS[i % PIE_COLORS.length],
                    }} />
                    <span style={{ flex: 1, color: 'var(--text-mid)' }}>
                      {allocView === 'fund'
                        ? (d.ticker && d.ticker !== '—' ? d.ticker : d.fullName || d.name)
                        : d.name
                      }
                    </span>
            ...
            <tbody>
              {sources.map(s => (
                <tr key={s.source}>
                  ...
                  <td style={{ textAlign: 'right' }} className={s.growth >= 0 ? 'gain' : 'loss'}>{fmt(s.growth)}</td>
